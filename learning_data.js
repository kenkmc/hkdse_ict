...
const learningData = {
    ...
    pages: {
        ...
        "tool-my-learning": {
            objectives: ["閱讀課堂瀏覽、練習、錯題及模擬考紀錄", "按證據找出今天可執行的下一步", "匯出或匯入只存於目前瀏覽器的學習備份"],
            concepts: [
                { icon: "續", label: "繼續學習", detail: "按最近瀏覽紀錄返回上一個課題，不必每次從目錄重新尋找。" },
                { icon: "證", label: "作答證據", detail: "得分率只由本站已作答題目計算；作答越少，判斷的不確定性越高。" },
                { icon: "弱", label: "弱項檢討", detail: "先重溫最低得分率課題，再用新題檢查能否把原理轉移到另一情境。" },
                { icon: "私", label: "本機私隱", detail: "資料不會上載；轉換瀏覽器或清除網站資料前，需要自行匯出備份。" }
            ],
            misconceptions: [
                { claim: "頁面顯示 80% 便等於公開試取得某個等級。", correction: "這只反映本站已完成題目的累積得分率，不包含正式卷難度、臨場表現、SBA 或等級界線。" },
                { claim: "GitHub Pages 會自動替我跨裝置同步進度。", correction: "沒有登入及後端服務時，localStorage 只屬於目前瀏覽器；要轉移需匯出再匯入。" }
            ],
            quickCheck: { question: "準備轉用另一部電腦前，應如何保留本站學習紀錄？", options: ["先匯出 JSON 備份，再在新瀏覽器匯入", "只關閉分頁", "只收藏首頁", "清除網站資料"], answerIndex: 0, explanation: "進度只在本機保存；匯出檔案才可由使用者自行轉移。" },
            examTip: "把弱項提示視為下一步練習方向，不要把單一百分比當成公開試等級預測。"
        },
        "tool-answer-lab": {
