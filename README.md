...
- 我的學習：把課題瀏覽、每次練習、今日重練、較弱課題、課業、遊戲及模擬考紀錄整理成可執行的下一步，並支援本機 JSON 備份與還原。
- DSE 課題答題重點：按 26 個必修及選修課題整理必用字詞、核心概念、常見題型、答題結構及失分位置。
- DSE 作答升級實驗室：提供試算表、網絡、SQL、算法及網頁保安共 15 項原創分層診斷／改寫／獨立作答任務。
...
- 共用學習助手：26 個課程頁均提供「起步、理解、互動、課業、應試」五步專注模式，可按需要一次顯示全部；內含零基礎扶手、字詞朗讀、圖像導讀、學習目標、課程界線、互動概念圖、常見誤解、快速檢查，以及由中央題庫載入的原創 DSE 題型與逐分準則。
...
- Viewport-safe 版面：首頁及 36 個內容頁共用桌面／手機防溢出規則；特別修正 SQL 全螢幕高度、Python 大畫布、模擬考黏附導覽、SBA 寬表格及 1080px 高桌面操作區。
...
- `assets/css/progress.css`：我的學習摘要、課題證據、模擬考歷史及備份介面。
...
- `assets/js/lesson_companion.js`：學習助手的五步專注模式、零基礎扶手、字詞朗讀、概念探索、即時檢查、分層課業及頁內 DSE 題型互動。
...
- `assets/js/progress_store.js`、`assets/js/progress_dashboard.js`：統一保存瀏覽、練習、課業、遊戲及模擬考事件，兼容舊有本機記錄，並產生「我的學習」摘要及備份。
- `assets/js/past-paper-index.js`：按課程單元呈現全部課題的用字、重點、題型、答題結構及失分位置。
- `assets/js/answer-lab.js`：五步核對、答案診斷、積木重組及高階自評互動。
- `assets/js/sba.js`：保存在目前瀏覽器的 SBA 證據清單、測試表產生器及成效指標。

內容及題目的永久 `id` 供錯題重練和日後完整進度系統使用。即使標題或檔名改變，也不應重用或隨意修改既有 ID。

目前版本沒有學生登入、雲端資料庫或自動跨裝置同步。課題瀏覽、練習事件、錯題、分層課業、SBA 清單、遊戲及模擬考紀錄只保存在該瀏覽器的 `localStorage`，不會上載 GitHub Pages；可在「我的學習」匯出及匯入 JSON 備份。清除網站資料前仍應先自行備份。
...
node scripts/validate_progress.js
```

目錄驗證會檢查重複識別碼、實際頁面、未列入目錄的 HTML，以及本機 `href`／`src`。題庫驗證會檢查題型、難度、課題連結、答案、評分準則、圖片檔、文字重複及目標題量；模擬卷驗證會檢查 3 份卷一的 40 + 60 分結構、每個選修的 2 組 25 分題組，以及圖像題和相連情境；錯題驗證會走完 1、3、7、14 日重練狀態；學習提示驗證會檢查 26 個課程頁的零基礎扶手、三級學習、圖像教材及 DSE 用字／重點；課題遊戲驗證會核對七款活動的掛載、關卡、參考答案、硬件圖片、版本、手機及 reduced-motion 支援；作答實驗室驗證會核對指令詞、5 組題材及 15 項分層任務；視窗版面驗證會檢查首頁和 36 個內容頁的共用修正版本；進度驗證會覆蓋瀏覽、每次作答、模擬考、舊記錄兼容、五步專注模式及備份還原。
